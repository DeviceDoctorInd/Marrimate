...
class FilterProfile extends StatefulWidget {
  ...
  FilterProfile({Key key, this.image, this.isProfilePicture = true}) : super(key: key);

  @override
  _FilterProfileState createState() => _FilterProfileState();
}
...
class _FilterProfileState extends State<FilterProfile> {
  ...
  uploadProfileImage()async{
    ...
    var response = await API.editProfile(userDetails: newDetails, newImage: true);
    if(response['status'] == true){
      Provider.of<UserModel>(context, listen: false).updateUserProfile(response['profile_image']);
      setLoading(false);
  ...
  uploadUserImage()async{
    ...
    await image.writeAsBytes(imageLib.encodeNamedImage(_image, widget.image.path.split("/").last));
    print(image.path);
    var response = await API.uploadUserImage(imagePath: image.path, userDetails: userDetails);
    if(response['status'] == true){
      Provider.of<UserModel>(context, listen: false).updateUserImage(response['data']);
      setLoading(false);
      Navigator.of(context).pop();
      Navigator.of(context).pop();
    }else{
          ...
          Scaffold(
            ...
            body: Column(
              children: [
                ...
                Padding(
                  padding: EdgeInsets.symmetric(
                      horizontal: padding * 4, vertical: padding),
                  child: MyButton(
                    onTap: () {
